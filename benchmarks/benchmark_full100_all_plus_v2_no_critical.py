...
RUN_NAME = "gemma_mmlu_full100_all_plus_v2_no_critical"
...
def coarse_pattern_best_router(model_choices: dict, judge_always_answer: str, gemma_final_arbiter_answer: str, sequential_answer: str) -> tuple[str, str]:
    """
    Post-hoc/leakage-inspired coarse pattern router.
    It does NOT see the correct answer at runtime.
    Mapping is based on prior full40 pattern analysis:
    all_same -> Gemma-Centered Sequential Review
    gemma_llama_agree -> Gemma Final Arbiter
    gemma_minority_vs_lm -> Judge Always
    gemma_mistral_agree -> Judge Always
    all_different -> Judge Always
    """
    ...
    if g == l and m != g and g in VALID_CHOICES:
        return gemma_final_arbiter_answer, "gemma_llama_agree -> Gemma Final Arbiter"
    if l == m and g != l and l in VALID_CHOICES:
        return judge_always_answer, "gemma_minority_vs_lm -> Judge Always"
    if g == m and l != g and g in VALID_CHOICES:
        return judge_always_answer, "gemma_mistral_agree -> Judge Always"
    return judge_always_answer, "all_different/invalid -> Judge Always"
...
def main():
    ...
    for index, item in enumerate(questions, start=1):
        ...
        try:
            start = time.time()
            judge_always_v2_raw = ask_ollama(JUDGE_MODEL, build_judge_always_prompt_v2(question, model_raw, model_choices, model_conf))
            judge_always_v2_time = round(time.time() - start, 2)
            judge_always_v2_answer = extract_choice(judge_always_v2_raw)
        except Exception as e:
            judge_always_v2_answer, judge_always_v2_raw, judge_always_v2_time = "?", f"ERROR: {e}", None
        ...
        gemma_final_v2_answer, gemma_final_v2_raw, gemma_final_v2_conf, gemma_final_v2_time = run_gemma_call(build_gemma_final_arbiter_prompt_v2(question, model_raw, model_choices, model_conf))
        ...
        coarse_pattern_answer, coarse_pattern_reason = coarse_pattern_best_router(model_choices, judge_always_answer, gemma_final_answer, sequential_answer)
        ...
        systems_to_print = [
            ...
            ("Judge Always v2", judge_always_v2_answer),
            ...
            ("Gemma Final Arbiter v2", gemma_final_v2_answer),
            ...
            ('Gemma-Centered Sequential Review', sequential_answer),
            ("Coarse Pattern-Best Router", coarse_pattern_answer),
            ("TriCheck Meta Decision", meta_answer),
        ]
        ...
        fields = [
            ...
            ("judge_always_v2", judge_always_v2_answer, judge_always_v2_time, judge_always_v2_raw),
            ...
            ("gemma_final_arbiter_v2", gemma_final_v2_answer, gemma_final_v2_time, gemma_final_v2_raw),
            ...
            ("coarse_pattern_best_router", coarse_pattern_answer, None, coarse_pattern_reason),
    ...
    systems.extend([
        ...
        {"system": "Judge Always v2", "correct_col": "judge_always_v2_correct", "time_col": "judge_always_v2_time_sec"},
        ...
        {"system": "Gemma Final Arbiter v2", "correct_col": "gemma_final_arbiter_v2_correct", "time_col": "gemma_final_arbiter_v2_time_sec"},
        ...
        {"system": "Coarse Pattern-Best Router", "correct_col": "coarse_pattern_best_router_correct", "time_col": None},
